...
for model_name in ASSETS:
    ...
    m1 = joblib.load(os.path.join(MODEL_PATH, f'xg_{model_name.lower()}_1.joblib'))
    m2 = joblib.load(os.path.join(MODEL_PATH, f'xg_{model_name.lower()}_1.joblib'))
    m3 = joblib.load(os.path.join(MODEL_PATH, f'xg_{model_name.lower()}_1.joblib'))
    model_list.append([m1, m2, m3])
    ...
    for asset_index in range(4):
        ...
        if ((counter+1) % DECISION_PERIOD == 0):    
            ...
            if memory.use_model[asset_index]:
                ...
                y2 = model_list[asset_index][0].predict(x)
                y3 = model_list[asset_index][0].predict(x)
